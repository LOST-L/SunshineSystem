...
<body style="width: 100%;height: 100%;">
    ...
    <script type="text/javascript">
        ...
        layui.use(['table', 'form'],function(){
            ...
            $(".addCategory").click(function () {
                    ...
                    btn1:function () {
                        ...
                        if(categoryId == ""){
                            ...
                            if(result.length == keyWords.length){
                                ...
                                $.ajax({
                                    url:"<%=basePath%>category_add.action",
                                    type:"post",
                                    data:{
                                        "wid":categoryId,
                                        "name": name,
                ...
                form.on('select(categoryLevelType)', function(data) {
                    ...
                    if(level != 0){
                        $.ajax({
                            url:"<%=basePath%>category_getList.action",
                            type:'get',
                            data:{
                                "grade":level
                            },
        ...
        function editCategory(data){
            removeErrorInput();
            $.ajax({
                url:"<%=basePath%>category_getInfo.action",
                type:"get",
                data:{
                    id : data.id
                },
                            ...
                            btn1:function () {
                                ...
                                if(name != ""){
                                    ...
                                    if(result.length == keyWords.length){
                                        var resultStr = JSON.stringify(result);
                                        $.ajax({
                                            url:"<%=basePath%>category_edit.action",
                                            type:"post",
                                            data:{
                                                "id":param.id,
                                                "name": name,
        ...
        function getTable(wid) {
            var url = "<%=basePath%>category_getList.action?grade=#grade";
            if(wid != 0 && wid != undefined){
                url = "<%=basePath%>category_getList.action?grade=#grade&pid="+wid;
            }
            if(urlFlag == "0"){
        ...
        function delCategory(sourceData){
            layer.confirm('您确定要删除这个类目吗？删除后将无法恢复数据', {
                ...
                $.ajax({
                    url: '<%=basePath%>category_del.action',
                    type: 'post',
                    data: {
                        "id" : sourceData.id,
                        "wid" : sourceData.wid
